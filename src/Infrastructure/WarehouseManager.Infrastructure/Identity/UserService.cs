...
using WarehouseManager.Application.Common.Models;
...
internal partial class UserService : IUserService
{
    ...
    public async Task ToggleStatusAsync(ToggleUserStatusRequestDto request, CancellationToken cancellationToken)
    {
        var user = await _userManager.Users.Where(u => u.Id == request.UserId).FirstOrDefaultAsync(cancellationToken);

        _ = user ?? throw new NotFoundException(_t["User Not Found."]);

        bool isAdmin = await _userManager.IsInRoleAsync(user, WHMRoles.Admin);
        if (isAdmin)
        {
